...
COMMENT "The raw coffee orders, ingested from orders-raw"
...
COMMENT "The cleaned coffee orders with valid order_id"
AS
  SELECT order_id, quantity, o.customer_id, c.profile:first_name as f_name, c.profile:last_name as l_name,
         cast(from_unixtime(order_timestamp, 'yyyy-MM-dd HH:mm:ss') AS timestamp) order_timestamp, o.coffee,
...
CREATE OR REFRESH LIVE TABLE cn_daily_customer_coffee
COMMENT "Daily number of coffee per customer in China"
AS
  SELECT customer_id, f_name, l_name, date_trunc("DD", order_timestamp) order_date, sum(quantity) coffee_counts
...
CREATE OR REFRESH LIVE TABLE fr_daily_customer_coffee
COMMENT "Daily number of coffee per customer in France"
AS
  SELECT customer_id, f_name, l_name, date_trunc("DD", order_timestamp) order_date, sum(quantity) coffee_counts
  FROM LIVE.orders_cleaned
